...
public class KThread {
	...
	if (currentThread.joinCalled) {

		KThread next = currentThread.joinThread;
	...
	Lib.assertTrue(this != currentThread);
	// ntdo - implement join() 
	if (!joinCalled && this != currentThread && (this.status == statusReady || this.status == statusBlocked)) {
		Machine.interrupt().disable();
		joinCalled = true;
		joinThread = currentThread;
		currentThread.waitQueue.acquire(currentThread);
		currentThread.waitQueue.waitForAccess(this);
		KThread.sleep();
		Machine.interrupt().enable();
	}
    }
...
//	new PingTest(0).run();
    ...
    //ntdo - add new boolean varible for join()
