...
def take_turn(plan, dice=make_fair_die(), who='Someone', comments=False):
    ...
    while not isOver:
        ...
        outcome = dice()
        score_for_turn, turn_total, isOver =  action(turn_total, outcome)
        if comments:
            commentate(action, outcome, score_for_turn, turn_total, isOver, who)
...
def take_turn_test():
    ...
    "*** YOUR CODE HERE ***"
    die = make_test_die(4, 6, 1)
    assert take_turn(plan, die) == 10
    die = make_test_die(1, 2, 3)
    assert take_turn(plan, die) == 1
    die = make_test_die(2, 1, 2)
    assert take_turn(plan, die) == 1
    die = make_test_die(2, 3, 1)
    assert take_turn(plan, die) == 1
    die = make_test_die(2, 3, 4, 2)
    assert take_turn(plan, die) == 11
    die = make_test_die(2, 2, 2, 2, 2, 2, 1)
    assert take_turn(plan, die) == 10
    die = make_test_die(2, 2, 2, 2, 2, 1)
    assert take_turn(plan, die) == 10
    die = make_test_die(2, 2, 2, 2, 1)
    assert take_turn(plan, die) == 1 
...
def describe_action(action):
    ...
    """
    "*** YOUR CODE HERE ***"
    if action != hold and action != roll:
...
def play(strategy, opponent_strategy):
    ...
    while score <= goal and opponent_score <= goal:
        if who == 0:
            ...
            plan = opponent_strategy(opponent_score, score)
            opponent_score += take_turn(plan)
        who = other(who)
    if score >= goal:
        who = 1
    else:
    ...
    return who # who is the winner, either 0 or 1
...
def run_strategy_experiments():
    ...
    "*** YOUR CODE HERE ***"
    ### problem 10
    print ("Best result from 15 to 25: ", eval_strategy_range(make_roll_until_strategy, 15, 25))

    ### problem 11a
    print ("Best result from 5 to 15: ", eval_strategy_range(make_die_specific_strategy, 5, 15))

    ### problem 11b
    print ("Best result from 0 to 10: ", eval_strategy_range(make_pride_strategy, 0, 10))
  
    ### problem 12
    ### should most of the time >= 0.6
    print ('win rate against the baseline:', compare_strategies(final_strategy))
    print ('win rate against the baseline:', compare_strategies(final_strategy))
    print ('win rate against the baseline:', compare_strategies(final_strategy))
    print ('win rate against the baseline:', compare_strategies(final_strategy))
    print ('win rate against the baseline:', compare_strategies(final_strategy))
...
def make_pride_strategy(margin, turn_goal=20):
    ...
    def strategy(score, opponent_score):
        ...
        if turn_goal >= margin - diff:
            return make_roll_until_plan(turn_goal)
        else:
            return make_roll_until_plan(margin - diff)
    return strategy
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    At our turn (regardless 4-sided or 6-sided die), we first try to get the total scores of 2 players divisible by 7 after our turn so that the opponent gets to roll the 4-sided die with higher risk. However, if the score is approaching the final goal (100 points), we only should roll until the turn total is at least the gap between the current score and the final score. In other words, take the shorter distance, either to get to the next multiple of 7 or to the final score. 
   
    >>> compare_strategies(final_strategy) > 0.6
    True
    """
    "*** YOUR CODE HERE ***"
    goal = 100
    sum_scores = score + opponent_score
    multiple_seven_after_sum = sum_scores
    while multiple_seven_after_sum % 7 != 0:
        multiple_seven_after_sum = multiple_seven_after_sum+ 1
    if multiple_seven_after_sum - sum_scores >= goal - score:
        return make_roll_until_plan(goal - score)
    else: 
        return make_roll_until_plan(multiple_seven_after_sum-sum_scores)
