...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        for c in range(0, i):
            if aPlace == hive:
                break
            aPlace = aPlace.entrance
        y = 0
        while aPlace != hive and y < k:
            
            if len(aPlace.bees) > 0:
                return random_or_none(aPlace.bees)
            aPlace = aPlace.entrance
            y += 1        
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0:
            for bee in self.place.bees[:]:
                bee.reduce_armor(self.fire_damage)
            print('{0} ran out of armor and expired'.format(self))
            self.place.remove_insect(self)
...
class BushAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        if self.ant != None and self.armor <= 0:
            self.ant.place.ant = self.ant
...
def apply_effect(effect, bee, duration):
    ...
    def new_action(colony):
        ...
        if duration > 0:
            duration = duration - 1
            return effect(old_action)(colony)
        else:
