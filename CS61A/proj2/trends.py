...
def analyze_tweet_sentiment(tweet):
    ...
    average = None
    "*** YOUR CODE HERE ***"
    i, sum, word_list = 0, 0, tweet_words(tweet)
    for word in word_list:
        if get_word_sentiment(word) != None:
            sum = sum + get_word_sentiment(word)
            i = i + 1
    if i != 0:
        average = sum / i
    return average
...
def find_centroid(polygon):
    ...
    """
    "*** YOUR CODE HERE ***"
    c_x, c_y, signed_area = 0, 0, 0
    for i in range(0, len(polygon)-1):
        temp = latitude(polygon[i]) * longitude(polygon[i+1]) - latitude(polygon[i+1]) * longitude(polygon[i])
        c_x = c_x + (latitude(polygon[i]) + latitude(polygon[i+1])) * temp
        c_y = c_y + (longitude(polygon[i]) + longitude(polygon[i+1])) * temp
        signed_area = signed_area + temp

    if signed_area == 0:
        return (latitude(polygon[0]), longitude(polygon[0]), signed_area)

    signed_area = signed_area / 2
    c_x = c_x / (6 * signed_area)
    c_y = c_y / (6 * signed_area)
    return (c_x, c_y, abs(signed_area))
...
def find_center(shapes):
    ...
    """
    "*** YOUR CODE HERE ***"
    sum_area, cxi_ai, cyi_ai  = 0, 0, 0
    for polygon in shapes:
        sum_area = sum_area + find_centroid(polygon)[2]
        cxi_ai = cxi_ai + find_centroid(polygon)[0] * find_centroid(polygon)[2]
        cyi_ai = cyi_ai + find_centroid(polygon)[1] * find_centroid(polygon)[2]
    c_x = cxi_ai / sum_area
    c_y = cyi_ai / sum_area
    return make_position(c_x, c_y)
...
def find_closest_state(tweet, state_centers):
    ...
    """
    "*** YOUR CODE HERE ***"
    current_location = tweet_location(tweet)
    closest_state = 'CA'
    closest_distance = geo_distance(current_location, state_centers['CA'])
    for state, center in state_centers.items():
        distance = geo_distance(current_location, center) 
...
def group_tweets_by_state(tweets):
    ...
    >>> sf = make_tweet("Welcome to San Francisco", None, 38, -122)
    >>> ssc = make_tweet("Welcome to Sacramento", None, 37.5, -122.5)
    >>> ny = make_tweet("Welcome to New York", None, 41, -74)
    >>> ca_tweets = group_tweets_by_state([sf, ny, ssc])['CA']
    >>> tweet_string(ca_tweets[1])
    '"Welcome to Sacramento" @ (37.5, -122.5)'
    """
    tweets_by_state = {}
    "*** YOUR CODE HERE ***"
    us_centers = {n: find_center(s) for n, s in us_states.items()}
    for tweet in tweets:
        ...
        if not closest_state in tweets_by_state: 
...
def calculate_average_sentiments(tweets_by_state):
    ...
    number_of_states = len(tweets_by_state)
    for each_state, tweets in tweets_by_state.items():
        how_many_tweets = len(tweets)
        sub_total = 0
        count_not_none_senti_tweet = 0
        for each_tweet in tweets: 
            if analyze_tweet_sentiment(each_tweet) !=None:
                count_not_none_senti_tweet = count_not_none_senti_tweet + 1
                sub_total = sub_total + analyze_tweet_sentiment(each_tweet)
        
            if count_not_none_senti_tweet != 0:
                averaged_state_sentiments[each_state] = sub_total / count_not_none_senti_tweet
