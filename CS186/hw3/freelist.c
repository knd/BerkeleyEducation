...
typedef struct BufferAccessStrategyData
{
	...
	/*
	 * True if the buffer just returned by StrategyGetBuffer had been in the * ring already.
	 */
...
StrategyGetBuffer(BufferAccessStrategy strategy, bool *lock_held)
{
	...
	*lock_held = true;
	...
	if (resultIndex == -1)
	{
		...
		{
			BufferDesc *temp = StrategyControl->head;
			while (temp != NULL) 
			{
				LockBufHdr(temp);
				if (temp->refcount == 0)
				{
					if (temp == StrategyControl->head && temp == StrategyControl->tail) 
					{
						StrategyControl->head = NULL;
						StrategyControl->tail = NULL;
					}
					else if (temp == StrategyControl->head) 
					{
						StrategyControl->head = temp->next;
						temp->next->prev = NULL;
					} 
					else if (temp == StrategyControl->tail) 
					{
						StrategyControl->tail = temp->prev;
						temp->prev->next = NULL;
					}
					else 
					{
		...
		{
			BufferDesc *temp = StrategyControl->tail;
			while (temp != NULL)
			{
				LockBufHdr(temp);
				if (temp->refcount == 0)
		...
		{
			double thres = floor(NBuffers / 2);
			if (StrategyControl->sizeFifo >= thres || StrategyControl->head == NULL) 
			{
				BufferDesc *temp = StrategyControl->tailFifo;
				while (temp != NULL) 
				{
					LockBufHdr(temp);
					if (temp->refcount == 0) 
					{
						if (temp == StrategyControl->headFifo && temp == StrategyControl->tailFifo) 
						{
							StrategyControl->headFifo = NULL;
							StrategyControl->tailFifo = NULL;
						}
						else if (temp == StrategyControl->headFifo) 
						{
							StrategyControl->headFifo = temp->nextFifo;
							temp->nextFifo->prevFifo = NULL;
						} 
						else if (temp == StrategyControl->tailFifo) 
						{
							StrategyControl->tailFifo = temp->prevFifo;
							temp->prevFifo->nextFifo = NULL;
						}
						else 
						{
							temp->prevFifo->nextFifo = temp->nextFifo;
							temp->nextFifo->prevFifo = temp->prevFifo;
						}
						temp->nextFifo = NULL;
						temp->prevFifo = NULL;
						StrategyControl->sizeFifo -= 1;
						resultIndex = temp->buf_id;
						UnlockBufHdr(temp);
						break;
					}
   ...
   * This function was added by the GSIs.
	...
	{	
		...
		else 
		{
			...
			// if not in linked list
			temp = buf;

			BufferDesc *buffer = StrategyControl->tail;
			StrategyControl->tail = temp;
			StrategyControl->tail->next = NULL;
			StrategyControl->tail->prev = buffer;
			buffer->next = StrategyControl->tail;
		}
	}
	//elog(LOG, "GRADING: UNPINNED %2d", buf->buf_id);  
	...
	else if (BufferReplacementPolicy == POLICY_2Q) 
	{
		...
		else  // *buf* is not on the Am queue 
		{
			temp = StrategyControl->headFifo;
			while (temp != NULL)
			{
				// if *buf* is on the A1 queue then
				if (temp == buf) 
				{
					StrategyControl->sizeFifo -= 1;
					if (temp == StrategyControl->headFifo && temp == StrategyControl->tailFifo) 
			...
			else // if *buf* is not in A1 and not in Am queues
			{
				StrategyControl->sizeFifo += 1;
				if (StrategyControl->headFifo == NULL)
				{
					StrategyControl->headFifo = buf;
					StrategyControl->tailFifo = buf;
					StrategyControl->headFifo->nextFifo = NULL;
					StrategyControl->headFifo->prevFifo = NULL;
				}
				else 
				{
					BufferDesc *buffer = StrategyControl->headFifo;
					StrategyControl->headFifo = buf;
					StrategyControl->headFifo->prevFifo = NULL;
					StrategyControl->headFifo->nextFifo = buffer;
					buffer->prevFifo = StrategyControl->headFifo;
				}
