...
void execute_one_inst(processor_t* p, int prompt, int print_regs)
{
  ...
  case 0x4: // opcode == 0x4 (BEQ)
	if (p->R[inst.itype.rs] == p->R[inst.itype.rt]) {
	  p->pc = p->pc + 4 + (int16_t)inst.itype.imm * 4;
	} else {
	  p->pc += 4;
	}
	break;
  ...
  case 0xa: // opcode == 0xa (SLTI)
	p->R[inst.itype.rt] = ((int16_t)p->R[inst.itype.rs] < (int16_t)inst.itype.imm);
	p->pc += 4;
	break;
  ...
  case 0xc: // opcode == 0xc (ANDI)
	p->R[inst.itype.rt] = (p->R[inst.itype.rs] & (uint32_t)inst.itype.imm);
	p->pc += 4;
	break;
  ...
  case 0x24: // opcode == 0x24 (LBU)
	p->R[inst.itype.rt] = (uint32_t)load_mem(p->R[inst.itype.rs]+(int16_t)inst.itype.imm, SIZE_BYTE);
	p->pc += 4;
	break;

  case 0x25: // opcode == 0x25 (LHU)
	p->R[inst.itype.rt] = (uint32_t)load_mem(p->R[inst.itype.rs]+(int16_t)inst.itype.imm, SIZE_HALF_WORD);
	p->pc += 4;
	break;
