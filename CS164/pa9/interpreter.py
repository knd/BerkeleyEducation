...
def Resume(state):
    ...
    def execGet():
        ...
        def get(obj):
            try: 
                # if not obj:
                #     return None
                if (isinstance(obj, tuple) and k < len(obj)) or (obj and k in obj):
                    return obj[k]
                if '__mt' in obj:
                    return get(obj['__mt'])
            except KeyError:
                print "KeyError in get(obj)"
                print "Error"
                sys.exit(-1)
    ...
    def execAddition():
        ...
        if isinstance(a, basestring) or isinstance(b, basestring):
            define(inst.ret, unicode(a) + unicode(b))
        else:
            define(inst.ret, a + b)
    def execDivision():
        a = lookup(inst.reg1)
...
def desugar(stmts):
    def desugarExp(e):
        if e[0] in ['+', '-', '*', '/', '==', '!=', '>', '<', '>=', '<=', 'in', 'get']:
            ...
            colonCall += 1
            tmp = colonCall
            h = desugarExp(('def', '__coloncall__' + str(tmp), desugarExp(e[1])))
            coloncall = ('var', '__coloncall__' + str(tmp))
