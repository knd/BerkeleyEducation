...
def runTest(cat, test, ref_output, test_output):
    try:
        if cat == 'ucalc':
            toRun = 'main_ucalc.py'
        elif cat == 'prolog':
            toRun = 'main_prolog.py'
        else:
            toRun = 'main_164.py'
        p = subprocess.Popen('python %s %s > %s' % (toRun, test, test_output), shell=True)
        runProc(p, timeout)
...
for (cat, tl) in tests.items():
    for test in tl:
        ...
        res = runTest(cat, testPath, ref_output, test_output)
        if (res == 0):
