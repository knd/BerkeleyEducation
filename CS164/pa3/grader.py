...
def runTest(test, ref_output, test_output):
    try:
        p = subprocess.Popen('python %s %s > %s' % (program, test, test_output), shell=True)
        runProc(p, timeout)
...
for cat in categories:
    tests[cat] = filter(lambda f: f.endswith(".p"), os.listdir(testDir + cat))
    scores[cat] = 0
    max_scores[cat] = 0
